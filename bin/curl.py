...
def main(args):
    ...
    ap.add_argument(
        "-o", "--output-file", help="write output to file instead of stdout"
    )
    ap.add_argument(
        "-O",
        "--remote-name",
    ...
    ap.add_argument(
        "-H", "--header", help="Custom header to pass to server (H)", action="append"
    )
    ...
    if ns.request_method == "GET":
        r = requests.get(url, headers=headers, allow_redirects=ns.location)
    elif ns.request_method == "POST":
        r = requests.post(
            url, data=ns.data, headers=headers, allow_redirects=ns.location
        )
