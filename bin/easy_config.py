...
class RGBColorPicker(object):
    ...
    def hexcode(self):
        ...
        hexc = "#%.02X%.02X%.02X" % (int(self.rgb_255[0]), int(
            self.rgb_255[1]), int(self.rgb_255[2]))
        return hexc
...
class ConfigView(ui.View):
    ...
    def tableview_cell_for_row(self, tv, section, row):
        ...
        if otype == TYPE_BOOL:
            ...
            def callback(s, self=self, i=i): return self.switch_changed(s, i)
            ...
            i = (sn, info["option_name"])
            def callback(s, self=self, i=i): return self.choice_changed(s, i)
            seg.action = callback
            ...
            i = (sn, info["option_name"])
            def callback(s, self=self, i=i): return self.choose_color(s, i)
            b.action = callback
            ...
            rawcolor = _stash.config.get(sn, info["option_name"])
            color = ast.literal_eval(rawcolor)
            rgb255color = int(color[0] * 255), int(color[1]
                                                   * 255), int(color[2] * 255)
            b.background_color = color
            b.title = "#%.02X%.02X%.02X" % rgb255color
            b.tint_color = ((0, 0, 0) if color[0] >= 0.5 else (1, 1, 1))
            i = (sn, info["option_name"])
            def callback(s, self=self, i=i): return self.choose_rgb_color(s, i)
            b.action = callback
            cell.content_view.add_subview(b)
            b.width = (cell.width / 6.0)
            b.height = ((cell.height / 4.0) * 3.0)
            ...
            tf.delegate = self
            i = (sn, info["option_name"])
            def callback(s, self=self, i=i): return self.str_entered(s, i)
            tf.action = callback
            if otype == TYPE_STR:
            ...
            if isinstance(cmd, string_types):
                def f(c=cmd): return _stash(c, add_to_history=False)
            else:
                def f(c=cmd): return cmd()

            def callback(s, self=self, f=f): return self.run_func(f)
            b.action = callback
            cell.content_view.add_subview(b)
            b.flex = "WH"
            b.frame = cell.frame
        ...
        if otype == TYPE_LABEL:
            ...
            console.alert(
                info.get(
                    ...
                    "value", ""), "Ok", hide_cancel_button=True)
        else:
            # show description
            console.alert(
                info.get(
