...
def open_temp(file="", new_tab=True):
    try:
        ...
        print(
            "***When you are finished editing the file, you must come back to console to confim changes***"
        )
        ...
        if input == "Y" or input == "y":
            while True:
                try:
                    save_as = (
                        raw_input("Save file as [Enter to confirm]: %s" % file_to_edit)
                        or file_to_edit
                    )
                except:
...
def open_editor(file="", new_tab=True):
    if os.path.isfile(os.getcwd() + "/" + file):
        ...
        editor.make_new_file(
            file if file else "untitled.py"
        )  # new_tab not supported by make_new_file
...
if __name__ == "__main__":
    ...
    ap.add_argument(
        "file", action="store", nargs="?", default=False, help="File to open"
    )
    ns = ap.parse_args()
