...
def unstage(commit='HEAD', paths=[]):
    ...
    for somepath in paths:
        ...
        try:
            ...
            # if tree_entry didnt exist, this file was being added, so remove
            # index entry
            try:
                del(index[path])
                index.write()
        ...
        try:
            ...
            # if index_entry doesnt exist, this file was being removed.  readd
            # it
            if os.path.exists(full_path):
                index_entry = list(
                    index_entry_from_stat(
                        posix.lstat(full_path),
                        tree_entry[1],
                        0))
            else:
...
def unstage_all(commit='HEAD'):
    # files to unstage consist of whatever was in new tree, plus whatever was
    # in old index (added files to old branch)
    repo = _get_repo().repo
    index = repo.open_index()
    tree_id = repo[commit]._tree
...
def git_reset(args):
    ...
    paths = ns.paths or []
    if not commit in repo and os.path.exists(
            commit):  # really specified a path
        paths = [commit] + paths
        commit = None
    ...
    if not paths:
        ...
        if commit == 'HEAD':
            ...
            repo.repo.refs.set_symbolic_ref(
                'HEAD', repo._format_ref_branch(commit))
        else:
    ...
    if ns.hard:
        ...
        for path in paths:
            ...
            file_contents = repo[treeobj.lookup_path(repo.__getitem__, relpath)[
                1]].as_raw_string()
...
def git_commit(args):
    ...
    ns.name = ns.name or get_config_or_prompt(
        repo, 'user', 'name', 'Author Name: ')
    ns.email = ns.email or get_config_or_prompt(
        repo, 'user', 'email', 'Author Email: ')
...
def git_clone(args):
    if len(args) > 0:
        url = args[0]
        repo = Gittle.clone(args[0], args[1] if len(
            args) > 1 else os.path.split(args[0])[-1].rstrip('.git'), bare=False)
...
def git_push(args):
    ...
    branch_name = os.path.join('refs', 'heads', repo.active_branch)

    print(
        "Attempting to push to: {0}, branch: {1}".format(
            result.url,
            branch_name))
...
commands = {
    'init': git_init, 'add': git_add, 'rm': git_rm, 'commit': git_commit, 'clone': git_clone, 'modified': git_modified, 'log': git_log, 'push': git_push, 'pull': git_pull, 'fetch': git_fetch, 'branch': git_branch, 'merge': git_merge, 'checkout': git_checkout, 'remote': git_remote, 'reset': git_reset, 'status': git_status, 'diff': git_diff, 'help': git_help
}
