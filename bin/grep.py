    ...
    def print_on_count(self):
        ...
        if self.count_only:
            if self.no_filename:
                ...
                print("{fn}:{cnt}".format(cnt=self.count, fn=self.filename_to_print))
...
class FileInputHandler(object):
    ...
    def __init__(
        self, filepaths, new_file_callback, stdin_label="<stdin>", encoding="latin1"
    ):
...
def main(args):
    ...
    ap.add_argument(
        ...
        "--line-number",
        action="store_true",
        help="prefix each line with the line number",
    )
    ap.add_argument(
        "-H",
        "--with-filename",
        action="store_true",
        help="prefix each line with the file name (default if multiple files are searched)",
    )
    ap.add_argument(
        "-h",
        "--no-filename",
        action="store_true",
        help="do not prefix each line with the file name (default if stdin or a single file is searched)",
    )
    ap.add_argument(
        "--label",
        action="store",
        default="<stdin>",
        help="label to use as file name for stdin",
    )
    ap.add_argument(
        "-c",
        "--count",
    ...
    ap.add_argument(
        ...
        "--files-without-match",
        action="store_true",
        help="only list file names of files that do not match",
    )
    ap.add_argument(
        "-l",
        "--files-with-matches",
        action="store_true",
        help="only list file names of files that match",
    )
    ap.add_argument(
        "-o",
        "--only-matching",
        action="store_true",
        help="only print the matching parts",
    )
    ...
    ap.add_argument(
        ...
        action="store",
        default="latin1",
        help="encoding to use when opening files (default encoding is latin1)",
    )
    ...
    try:
            ...
            for f in ns.files:
                if f == "-":
                    ...
                    if p.is_file():
                        ...
                        print(
                            "grep: {} was skipped because it does not exist.".format(f),
                            file=sys.stderr,
                        )
                    else:
                        print(
                            "grep: {} was skipped because it is not a file.".format(f),
                            file=sys.stderr,
                        )
            if len(filepaths) == 0:
        ...
        if ns.no_filename:
            ...
            if ns.with_filename:
                print(
                    "grep: option --with-filename ignored since --no-filename was specified as well.",
                    file=sys.stderr,
                )
        ...
        count_output_handler = CountOutputHandler(
            ns.files_without_match, ns.files_with_matches, no_filename, ns.count
        )
        file_input_handler = FileInputHandler(
            filepaths, count_output_handler.new_file, ns.label, ns.encoding
        )
        ...
        for line in file_input_handler.input():
            if bool(pattern.search(line)) != ns.invert:
                ...
                if ns.files_with_matches:
                    if (
                        not ns.invert and not ns.count
                    ):  # if we do not need a count, skip the rest of the file
                        file_input_handler.nextfile()
                elif not (ns.count or ns.files_without_match):
                    if ns.invert:
                    ...
                    for ln in lines:
                        print(
                            fmt.format(
                                filename=count_output_handler.filename_to_print,
