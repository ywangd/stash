...
def main(args):
    ...
    ap.add_argument(
        "-1", "--one-line", action="store_true", help="List one file per line"
    )
    ap.add_argument(
    ...
    ap.add_argument(
        ...
        "--human",
        action="store_true",
        help="display size in human legible format",
    )
    ap.add_argument(
        "-d",
        "--dirs",
        action="store_true",
        help="list directories instead of their content",
    )
    ...
    format_size = human_legible_size if ns.human else unformatted_size
        ...
        def _fmt(filename, dirname=""):
            ...
            if os.path.islink(fullpath):
                filename = _stash.text_color(
                    filename, "yellow"
                ) + " -> %s" % _stash.libcore.abbreviate(
                    os.path.realpath(fullpath), relativeTo=os.path.dirname(fullpath)
                )
    ...
    if len(ns.files) == 0:
        ...
        for f in sorted(ns.files, key=sortKey):
            if not os.path.exists(f):
                ...
                exitcode = 1
            elif os.path.isdir(f) and not ns.dirs:
