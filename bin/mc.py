...
class McCmd(cmd.Cmd):
    ...
    def do_disconnect(self, command):
        ...
        if len(args) != 1:
            self.stdout.write(
                Text(
                    "Error: expected exactly on argument!\n",
                    "red"))
            return
        ID = args[0]
        ...
        if ID == INTERN_FS_ID:
            self.stdout.write(
                Text(
                    "Error: cannot close internal FSI!\n",
                    "red"))
            return
        try:
            self.FSIs[ID].close()
    ...
    def do_cd(self, command):
        ...
        if name == "..":
            ...
            try:
                ...
                isdir = True
                # lets just try. It worked before isdir() was added so it
                # should still work
        if not isdir:
            self.stdout.write(
                Text(
                    "Error: dirname does not refer to a dir!\n",
                    "red"))
            return
        try:
            fsi.cd(name)
            ...
            try:
                if mode in ("w", "W"):
                    ...
                    if moded:
                        ...
                        cpcmd = rawcpcmd.format(
                            lfp=lfp, ri=rid, tp=tp, li=INTERN_FS_ID)
                        self.do_cp(cpcmd)
