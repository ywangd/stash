...
BLOCKLIST_PATH = os.path.join(
    os.path.expandvars("$STASH_ROOT"), "data", "pip_blocklist.json"
)
...
def print_info(
    package, pip_info_file=PIP_INFO_FILE, site_packages=SITE_PACKAGES_FOLDER
):
...
def download_info(
    pkg_name, pip_info_file=PIP_INFO_FILE, site_packages=SITE_PACKAGES_FOLDER
):
...
def update_req_index(
    ...
    for package, info in info_list:
        ...
        if os.path.exists(info_file):
            ...
            print(
                _stash.text_color(
                    "Info file of Package {} not found".format(package), "yellow"
                )
            )
    ...
    def _run_setup_file(self, filename, extras=[]):
        ...
        for p in sorted(packages):  # folders or files under source root
            if p == "":  # no packages just files
                ...
                if use_2to3:
                    _stash(
                        """find {} --name '.py' | xargs -n 1 -I %% 2to3 -w %% > /dev/null""".format(
                            target_dir
                        )
                    )
...
class PackageRepository(object):
    ...
    def _install(
        ...
        for dep_name, ver_spec, extras in name_versions:
            ...
            try:
                repository.install(
                    dep_name, ver_spec, flags=dependency_flags, extras=extras
                )
...
class PyPIRepository(PackageRepository):
    ...
    def download(self, pkg_name, ver_spec, flags=DEFAULT_FLAGS):
        ...
        if source is not None and (flags & FLAG_DIST_ALLOW_SRC > 0):
            ...
            if (wheel is None or (flags & FLAG_DIST_ALLOW_WHL == 0)) or (
                flags & FLAG_DIST_PREFER_SRC > 0
            ):
    ...
    def install(
        ...
        if blocklisted and not (flags & FLAG_IGNORE_BLOCKLIST > 0):
            if fatal:
                ...
                print(
                    _stash.text_color(
                        "Package {} is blocklisted and marked fatal. Failing install.".format(
                            pkg_name
                        ),
                        "red",
                    )
                ...
                print(
                    _stash.text_color(
                        "Warning: package '{}' is blocklisted, but marked as non-fatal.".format(
                            pkg_name
                        ),
                        "yellow",
                    )
...
class GitHubRepository(PackageRepository):
    ...
    def download(self, owner_repo, ver_spec):
        ...
        _stash(
            "wget https://github.com/{0}/{1}/archive/{2}.zip -o $TMPDIR/{2}.zip".format(
                owner, repo, release
            )
        )
...
class LocalRepository(PackageRepository):
    ...
    def install(self, archive_filename, ver_spec, flags=DEFAULT_FLAGS, extras=[]):
        ...
        self._install(
            pkg_name, pkg_info, archive_filename, dependency_flags=flags, extras=extras
        )
    ...
    try:
        if ns.sub_command == "list":
            ...
            for module, info in info_list:
                print(
                    "{} ({}) - {}".format(
                        module, info.get("version", "???"), info.get("summary", "")
                    )
                )
