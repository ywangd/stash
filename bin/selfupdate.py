...
def main(args):
    ...
    print(
        _stash.text_style(
            "Running selfupdate ...", {"color": "yellow", "traits": ["bold"]}
        )
    )
    print("%s: %s:%s" % (_stash.text_bold("Target"), owner, branch))
    ...
    if not ns.check and has_update:
        ...
        try:
            ...
            print(
                _stash.text_color(
                    "Please restart Pythonista to ensure changes becoming effective.",
                    "green",
                )
            )
