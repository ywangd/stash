...
def branch(args):
    ...
    list_grp.add_argument(
        "-a", "--all", action="store_true", help="list both remote and local branches"
    )
    ...
    if not result.remotes:
        for key, value in repo.branches.items():
            ...
            if tracking:
                ...
                diffmsg = (
                    "+{}/-{} compare to".format(ahead, behind) if result.verbose else ""
                )
                trackmsg = "[{} {} {}]".format(diffmsg, tracking, trackingsha[0:N])
            print(
                " ".join(
                    [
                        ("* " if repo.active_branch == key else "") + key,
                        dispval,
                        commitmsg,
                    ]
                )
        ...
        if commits_ahead:
            raise GitError(
                "{0} is ahead of {1} by {2} commits.\nuse git branch -D\n".format(
                    delete_branchname, remote_tracking_branch, commits_ahead
                )
            )
    print(
...
def move_branch(movebranch, force, verbose):
    ...
    if newbranch in repo.branches and not force:
        raise GitError(
            "{} already exists.  use -M to force overwriting".format(newbranch)
        )
    if newbranch != oldbranch:
        print(
            "Renaming {} ({}) to {}\n".format(
                oldbranch, repo.branches[oldbranch], newbranch
            )
        )
        repo.add_ref(
            repo._format_ref_branch(newbranch), repo._format_ref_branch(oldbranch)
        )
...
def create_branch(new_branch, base_rev, force=False, no_track=False):
    ...
    tracking, remote_branch = (["origin"] + base_rev.split("/"))[
        -2:
    ]  # branch-> origin/branch.  remote/branch stays as is
    qualified_remote_branch = os.path.sep.join([tracking, remote_branch])
    if (
        qualified_remote_branch in repo.remote_branches
        and base_rev not in repo.branches
    ):
        if not no_track:
