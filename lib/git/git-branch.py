...
def branch(args):
    ...
    add_branchname = (
        result.branchname,
        result.startpoint or repo.active_branch)
    ...
    if list_flag:
        ...
        add_tracking(set_upstream[0], *(['origin'] +
                                        set_upstream[1].split('/'))[-2:])
        print(
            set_upstream[0],
            format_tracking_branch_desc(
                repo,
                set_upstream[0]))
    elif no_track:
        if len(no_track) == 1:
...
def format_tracking_branch_desc(repo, branchname):
    try:
        ...
        return '+{}/-{} relative to {} ({})'.format(ahead,
                                                    behind, remote, theirsha)
    except KeyError:
        return ''
    ...
    if not result.remotes:
        for key, value in iteritems(repo.branches):
            ...
            if tracking:
                ...
                trackmsg = '[{} {} {}]'.format(
                    diffmsg, tracking, trackingsha[0:N])
            print (' '.join(
                [('* ' if repo.active_branch == key else '') + key, dispval, commitmsg]))
...
def move_branch(movebranch, force, verbose):
    ...
    if newbranch != oldbranch:
        print(
            'Renaming {} ({}) to {}\n'.format(
                oldbranch,
                repo.branches[oldbranch],
                newbranch))
        repo.add_ref(
            repo._format_ref_branch(newbranch),
            repo._format_ref_branch(oldbranch))
...
def create_branch(new_branch, base_rev, force=False, no_track=False):
    ...
    tracking, remote_branch = (['origin'] + base_rev.split('/'))[-2:]
