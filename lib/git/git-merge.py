...
def _merge_entries(path, trees):
    ...
    while any([ind < l for ind, l in zip(inds, lens)]):
        ...
        inds = [
            ind + 1 if e.path == minpath else ind for e, ind in zip(next_entry, inds)
        ]
...
def walk_trees(store, tree_ids, prune_identical=False):
    ...
    while todo:
        ...
        trees = [
            is_tree and store[entry.sha] or None
            for is_tree, entry in zip(is_trees, entries)
        ]
...
def merge(args):
    ...
    parser.add_argument(
        "--msg", nargs=1, action="store", help="commit message to store"
    )
    parser.add_argument(
        "--abort", action="store_true", help="abort in progress merge attempt"
    )
    ...
    merge_head = find_revision_sha(
        repo, result.commit or get_remote_tracking_branch(repo, repo.active_branch)
    )
    if not merge_head:
        raise GitError(
