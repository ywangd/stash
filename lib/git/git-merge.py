...
def _merge_entries(path, trees):
    ...
    while any([ind < l for ind, l in zip(inds, lens)]):
        next_entry = [
            e[ind] if ind < l else _NULL_ENTRY for e,
            ind,
            l in zip(
                entries,
                inds,
        ...
        inds = [
            ind +
            1 if e.path == minpath else ind for e,
            ind in zip(
                next_entry,
                inds)]
...
def walk_trees(store, tree_ids, prune_identical=False):
    ...
    todo = [[TreeEntry(b'', mode, tree_id)
             for mode, tree_id in zip(modes, tree_ids)]]
    ...
    while todo:
        ...
        trees = [is_tree and store[entry.sha]
                 or None for is_tree, entry in zip(is_trees, entries)]
...
def merge_trees(store, base, mine, theirs):
    ...
    for b, m, t in w:
        ...
        # if base==mine, but not theirs, delete
            ...
            result = diff3.merge(store[m.sha].data.splitlines(True), store[b.sha].data.splitlines(
                True) if b.sha else [''], store[t.sha].data.splitlines(True))
...
def merge(args):
    ...
    parser.add_argument(
        ...
        nargs=1,
        action='store',
        help='commit message to store')
    parser.add_argument(
        '--abort',
        action='store_true',
        help='abort in progress merge attempt')
    ...
    merge_head = find_revision_sha(
        repo, result.commit or get_remote_tracking_branch(
    ...
    base_sha = merge_base(repo, head, merge_head)[
        0]  # fixme, what if multiple bases

    if base_sha == head:
        print(
            'Fast forwarding {} to {}'.format(
                repo.active_branch,
                merge_head))
    ...
    print('merging <{}> into <{}>\n{} commits ahead of merge base <{}> respectively'.format(
        merge_head[0:7], head[0:7], count_commits_between(repo, merge_head, head), base_sha[0:7]))
    base_tree = repo[base_sha].tree
    merge_head_tree = repo[merge_head].tree
    head_tree = repo[head].tree

    num_conflicts, added, removed = merge_trees(
        repo.repo.object_store, base_tree, head_tree, merge_head_tree)
    # update index
    if added:
    ...
    repo.repo._put_named_file(
        'MERGE_MSG', 'Merged from {}({})'.format(
            merge_head, result.commit))
