...
class GetstashTests(StashTestCase):
    ...
    def test_install_pythonista(self):
        ...
        self.run_getstash(
            install_path=sd, launcher_path=lp, zippath=zp, dist="pythonista"
        )
        expected = [
            "bin",
            "system",
        ...
        self.assertTrue(
            os.path.exists(sd), "StaSh base directory not found after install!"
        )
        self.assertTrue(
            os.path.exists(lp), "'launch_stash.py' not found after install!"
        )
        for fn in expected:
