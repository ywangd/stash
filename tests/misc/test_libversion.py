...
class LibVersionTests(StashTestCase):
    ...
    def test_version_specifier_parse(self):
        ...
        to_test = [
            ...
            ("with1number == 923.1512.12412", "with1number",
             [(operator.eq, "923.1512.12412")], []),
            ...
            ("nondigitver <= 1.5.3b", "nondigitver",
             [(operator.le, "1.5.3b")], []),
            ("wrapt < 2, >= 1", "wrapt", [
             (operator.lt, "2"), (operator.ge, "1")], []),
            ("extras[simple]", "extras", None, ["simple"]),
            ("extras[multi, values]", "extras", None, ["multi", "values"]),
            ("extras == 9.8.7 [with_version]", "extras",
             [(operator.eq, "9.8.7")], ["with_version"]),
        ]
        for req, pkg, spec, exp_extras in to_test:
            name, ver_spec, extras = self.stash.libversion.VersionSpecifier.parse_requirement(
                req)
    ...
    def test_version_specifier_match(self):
        ...
        to_test = [
            ...
            ("eqtest == 1.0.0", [("1.0.0", True), ("1.0.1", False), ("1.0.0.0",
                                                                     False), ("11.0.0", False), ("0.1.0.0", False), ("0.9.0", False)]),
            ("lttest <= 2.0.0", [("2.0.0", True), ("2.0.1", False), ("3.0.0", False), (
                "1.0.0", True), ("1.9.0", True), ("11.0.0", False), ("1.9.2b", True)]),
            ("gttest >= 3.5.0", [("3.5.0", True), ("3.4.9", False), ("3.6.0",
                                                                     True), ("11.0.0", True), ("3.5.0a", False), ("1.0.0", False)]),
            ("eqstr == str", [("1.0.0", False), ("str", True),
                              ("str2", False), ("s", False), ("99.999.99", False)]),
            ("wrapt < 2, >= 1", [("0.0.1", False), ("1.0.0", True),
                                 ("1.5.0", True), ("2.0.0", False), ("1.9.9", True)]),
        ]
        for rs, test in to_test:
            _, ver_spec, extras = self.stash.libversion.VersionSpecifier.parse_requirement(
                rs)
    ...
    def test_version_parse(self):
        ...
        to_test = [
            ...
            ("1.2.3", {"epoch": 0, "versiontuple": (
                1, 2, 3), "is_postrelease": False}),
            ("1!2.3", {"epoch": 1, "versiontuple": (
                2, 3), "is_devrelease": False}),
            ("5.5.4a.post5",
             {"versiontuple": (5,
                               5,
                               4),
            ...
            ("0.8.4.post.dev", {"versiontuple": (
                0, 8, 4), "is_postrelease": True, "postrelease": 0, "is_devrelease": True}),
        ]
